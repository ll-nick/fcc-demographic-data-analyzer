...
def calculate_demographic_data(print_data=True):
    ...
    percentage_bachelors = round(100 * df[df["education"] == "Bachelors"].shape[0] / total_count, 1)

    # What percentage of people with advanced education (`Bachelors`, `Masters`, or `Doctorate`) make more than 50K?
    # What percentage of people without advanced education make more than 50K?

    # with and without `Bachelors`, `Masters`, or `Doctorate`
    # higher_education = round(df[(df["education"].isin(["Bachelors", "Masters", "Doctorate"])) & (df["salary"] == ">50K")].shape[0], 1)
    # lower_education = round(df[~(df["education"].isin(["Bachelors", "Masters", "Doctorate"])) & (df["salary"] == ">50K")].shape[0], 1)
    ...
    num_rich_min_workers = min_workers[min_workers["salary"] == ">50K"].shape[0]

    rich_percentage = round(100 * num_rich_min_workers / num_min_workers, 1)
